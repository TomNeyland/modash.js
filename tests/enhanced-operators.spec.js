...
describe('Enhanced MongoDB Operators', () => {
    const testData = [
        { _id: 1, name: 'Alice', age: 30, tags: ['developer', 'senior'], scores: [85, 90, 88] },
        { _id: 2, name: 'Bob', age: 25, tags: ['designer'], scores: [92, 87] },
        { _id: 3, name: 'Charlie', age: 35, tags: ['developer', 'lead'], scores: [78, 85, 82, 90] },
    ...
    describe('Enhanced $match operators', () => {
        ...
        it('should handle logical $and operator', () => {
            const result = Modash.aggregate(testData, [
                { $match: { $and: [{ age: { $gte: 30 } }, { tags: { $exists: true } }] } }
            ]);
            expect(result).to.have.length(2);
            expect(result.map(r => r.name)).to.deep.equal(['Alice', 'Charlie']);
        });
    ...
    describe('Enhanced Array Operators', () => {
        ...
        describe('$concatArrays', () => {
            it('should concatenate arrays', () => {
                ...
                expect(result[0].combined).to.deep.equal(['prefix', 'developer', 'senior', 'suffix']);
                expect(result[1].combined).to.deep.equal(['prefix', 'designer', 'suffix']);
            });
        });

...
describe('Streaming vs Non-Streaming Equivalence Tests', () => {
  ...
  const exhaustivelyCompareResults = (collection, pipeline, description = '') => {
    ...
    expect(aggregateStreamingResult).to.deep.equal(nonStreamingResult, 
      `aggregateStreaming(array) result differs from non-streaming for: ${description}`);
    expect(aggregateStreamingCollectionResult).to.deep.equal(nonStreamingResult, 
  ...
  describe('Core Pipeline Stages Equivalence', () => {
    it('should produce identical results for $match operations', () => {
      const pipelines = [
        [{ $match: { qty: { $gte: 250 } } }],
        [{ $match: { item: 'abc1' } }],
        [{ $match: { $and: [{ qty: { $gt: 100 } }, { item: { $regex: 'abc.*' } }] } }],
        [{ $match: { tags: { $exists: true } } }]
      ];

      pipelines.forEach((pipeline, index) => {
        exhaustivelyCompareResults(testData.inventory, pipeline, `$match test ${index + 1}`);
      });
    });
    ...
    it('should produce identical results for $project operations', () => {
      ...
      pipelines.forEach((pipeline, index) => {
        exhaustivelyCompareResults(testData.inventory, pipeline, `$project test ${index + 1}`);
      });
    });
    ...
    it('should produce identical results for $group operations', () => {
      const pipelines = [
        ...
        [{ $group: { _id: '$status', items: { $push: '$item' }, maxQty: { $max: '$qty' } } }]
      ];

      pipelines.forEach((pipeline, index) => {
        // For $group, we need to sort results since order is not guaranteed
        const results = exhaustivelyCompareResults(testData.inventory, pipeline, `$group test ${index + 1}`);
        
        // Verify the groups are equivalent by sorting them
        const sortById = (arr) => [...arr].sort((a, b) => {
        ...
        expect(sortById(results.streaming)).to.deep.equal(sortById(results.nonStreaming));
      });
    });
    ...
    it('should produce identical results for $sort operations', () => {
      ...
      pipelines.forEach((pipeline, index) => {
        exhaustivelyCompareResults(testData.inventory, pipeline, `$sort test ${index + 1}`);
      });
    });
    ...
    it('should produce identical results for $limit and $skip operations', () => {
      ...
      pipelines.forEach((pipeline, index) => {
        exhaustivelyCompareResults(testData.inventory, pipeline, `limit/skip test ${index + 1}`);
      });
    });
  });
  ...
  describe('Complex Multi-Stage Pipeline Equivalence', () => {
    it('should produce identical results for complex sales analysis pipeline', () => {
      ...
      exhaustivelyCompareResults(testData.sales2, complexPipeline, 'complex sales analysis');
    });
    ...
    it('should produce identical results for inventory analysis with mathematical operations', () => {
      ...
      exhaustivelyCompareResults(testData.inventory, mathPipeline, 'inventory with math operations');
    });
    ...
    it('should produce identical results for text processing pipeline', () => {
      const textPipeline = [
        {
          $project: {
            ...
            description: { $concat: ['Item: ', '$item', ' (Qty: ', { $toString: '$qty' }, ')'] }
          }
        },
      ...
      exhaustivelyCompareResults(testData.inventory, textPipeline, 'text processing');
    });
  });
  ...
  describe('Edge Cases Equivalence', () => {
    it('should handle empty collections identically', () => {
      ...
      pipelines.forEach((pipeline, index) => {
        exhaustivelyCompareResults([], pipeline, `empty collection test ${index + 1}`);
      });
    });
    ...
    it('should handle null and undefined values identically', () => {
      ...
      pipelines.forEach((pipeline, index) => {
        exhaustivelyCompareResults(testDataWithNulls, pipeline, `null handling test ${index + 1}`);
      });
    });
    ...
    it('should handle array operations identically', () => {
      ...
      const pipelines = [
        [{ $match: { tags: { $size: 3 } } }],
        [{ $project: { firstTag: { $arrayElemAt: ['$tags', 0] }, avgScore: { $avg: '$scores' } } }],
        [{ $match: { tags: { $all: ['blue'] } } }],
        [{ $project: { tagCount: { $size: { $ifNull: ['$tags', []] } } } }]
      ];

      pipelines.forEach((pipeline, index) => {
        exhaustivelyCompareResults(arrayTestData, pipeline, `array operations test ${index + 1}`);
      });
    });
  });
  ...
  describe('Performance Consistency', () => {
    it('should maintain consistent performance characteristics between streaming and non-streaming', () => {
      ...
      const nonStreamingResult = Modash.aggregate(largeDataset, complexPipeline);
      ...
      const sortByCategory = (arr) => [...arr].sort((a, b) => a._id.localeCompare(b._id));
      expect(sortByCategory(streamingResult)).to.deep.equal(sortByCategory(nonStreamingResult));

      // Performance should be reasonable (streaming shouldn't be dramatically slower)
      expect(streamingTime).to.be.lessThan(nonStreamingTime * 3, 
        `Streaming took ${streamingTime}ms vs non-streaming ${nonStreamingTime}ms`);
      
      console.log(`      Performance: Non-streaming: ${nonStreamingTime.toFixed(2)}ms, Streaming: ${streamingTime.toFixed(2)}ms`);
    });
  });
  ...
  describe('Live Update Correctness', () => {
    it('should maintain correctness when adding data to streaming collections', () => {
      ...
      const sortByTotalQty = (arr) => [...arr].sort((a, b) => (b.totalQty || 0) - (a.totalQty || 0));
      expect(sortByTotalQty(streamingResult)).to.deep.equal(sortByTotalQty(completeResult));
    });
  });
});
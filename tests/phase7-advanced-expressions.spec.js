...
describe('Phase 7: Advanced Expression & Accumulator Support', () => {
  const testData = [
    { name: 'Alice', age: 30, score: 85, status: 'active', tags: ['admin', 'user'] },
    { name: 'Bob', age: 25, score: 92, status: 'inactive', tags: ['user'] },
    { name: 'Charlie', age: 35, score: null, status: 'active', tags: ['admin', 'manager'] },
    { name: 'Diana', age: 28, score: 78, status: null, tags: ['user', 'guest'] }
  ];
  ...
  describe('$reduce operator with $$value and $$this', () => {
    ...
    it('should handle complex accumulation scenarios', () => {
      const result = $project([
        { items: [{ price: 10, qty: 2 }, { price: 20, qty: 1 }, { price: 5, qty: 3 }] }
      ], {
        totalValue: {
          $reduce: {
            input: '$items',
      ...
      const result = $project(testData, {
        ...
        analysis: {
          $switch: {
            branches: [
              {
                case: { $and: [{ $isNumber: '$score' }, { $gte: ['$score', 90] }] },
                then: { 
                  $mergeObjects: [
                    { level: 'excellent' },

...
function $project(collection, specifications) {
    ...
    projection = chain(collection).map(function(obj, key, list) {

        var result = {};

        for (let field in specifications) {
            let expression = specifications[field];

            if (typeof expression === 'bool' || expression === 1 || expression === 0) {
                // Simple passthrough of field/path values
                expression = '$' + field;
            }

...
function estimateCardinality(
  ...
  idExpression: CompiledExpression
): number {
  ...
  if (typeof spec === 'object' && spec !== null && !Array.isArray(spec)) {
    ...
    switch (operator) {
      ...
      case '$addToSet':
        ...
        return {
          type: AccumulatorType.ADD_TO_SET,
...
export function performanceGroup(
  ...
  const idExpression = groupSpec._id
    ? compileExpression(groupSpec._id, ctx)
    : () => null;
  ...
  for (let i = 0; i < collection.length; i++) {
    ...
    for (const accumulator of compiledAccumulators) {
      const value = accumulator.expression
        ? accumulator.expression(doc, i)
        : null;
      ...
      switch (accumulator.type) {
        ...
        case AccumulatorType.MIN:
          if (typeof value === 'number' && !isNaN(value)) {
            accumulators.min[groupIndex] = Math.min(
              accumulators.min[groupIndex],
              value
            );
          }
          break;
        ...
        case AccumulatorType.MAX:
          if (typeof value === 'number' && !isNaN(value)) {
            accumulators.max[groupIndex] = Math.max(
              accumulators.max[groupIndex],
              value
            );
          }
          break;
    ...
    for (const accumulator of compiledAccumulators) {
      switch (accumulator.type) {
        ...
        case AccumulatorType.AVG:
          ...
          result[accumulator.fieldName] =
            count > 0 ? accumulators.sum[groupIndex] / count : 0;
          break;
        ...
        case AccumulatorType.PUSH:
          result[accumulator.fieldName] =
            accumulators.arrays?.get(groupIndex) || [];
          break;

        case AccumulatorType.ADD_TO_SET:
          const setValues = accumulators.sets?.get(groupIndex);
          result[accumulator.fieldName] = setValues

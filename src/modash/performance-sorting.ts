...
import { compileFieldAccess, type CompilationContext } from './performance-compiler.js';
...
class TopKHeap {
  ...
  constructor(k: number, compareFn: (a: HeapNode, b: HeapNode) => number, findMax: boolean = true) {
    this.capacity = k;
    this.heap = [];
  ...
  private heapifyDown(index: number): void {
    while (this.leftChild(index) < this.heap.length) {
      ...
      if (this.compareFn(this.heap[index]!, this.heap[smallestChild]!) <= 0) break;
 ...
 */
function createComparator(compiledKeys: CompiledSortKey[]): (a: HeapNode, b: HeapNode) => number {
 ...
 */
function computeSortKey(doc: Document, compiledKeys: CompiledSortKey[], rowId: number): any[] {
  const sortKey: any[] = [];
 ...
 */
export function canUseTopK(pipeline: any[], startIndex: number): { canUse: boolean; limit?: number } {
  ...
  if (currentStage && '$sort' in currentStage && nextStage && '$limit' in nextStage) {

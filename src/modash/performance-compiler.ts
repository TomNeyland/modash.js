...
import type { Pipeline, PipelineStage, QueryExpression, Expression } from '../index.js';
import { perfCounters } from '../../benchmarks/operators.js';

// Compiled expression function type
export type CompiledExpression = (doc: Document, rowId?: number) => DocumentValue;
 ...
 */
export function compileFieldAccess(fieldPath: string, ctx: CompilationContext): (doc: Document) => DocumentValue {
  // Check cache first
 ...
 */
function compileRegex(pattern: string, options?: string, ctx?: CompilationContext): RegExp {
 ...
 */
function foldConstants(expression: Expression, ctx: CompilationContext): DocumentValue | null {
  ...
  if ('$add' in expression && Array.isArray(expression.$add)) {
    ...
    if (values.every(v => typeof v === 'number')) {
      const result = values.reduce((sum: number, val) => sum + (val as number), 0);
  ...
  if ('$multiply' in expression && Array.isArray(expression.$multiply)) {
    ...
    if (values.every(v => typeof v === 'number')) {
      const result = values.reduce((product: number, val) => product * (val as number), 1);
      ctx.constants.set(key, result);
      return result;
    }
  }
  
  if ('$subtract' in expression && Array.isArray(expression.$subtract) && expression.$subtract.length === 2) {
  ...
  if ('$divide' in expression && Array.isArray(expression.$divide) && expression.$divide.length === 2) {
 ...
 */
export function compileMatch(query: QueryExpression, ctx: CompilationContext): CompiledPredicate {
  const compiledConditions: Array<(doc: Document) => boolean> = [];
  
  for (const [field, condition] of Object.entries(query)) {
    // Handle logical operators
    if (field === '$and' && Array.isArray(condition)) {
      const subPredicates = condition.map(subQuery => compileMatch(subQuery as QueryExpression, ctx));
      compiledConditions.push((doc: Document) => subPredicates.every(pred => pred(doc)));
      continue;
    }
    
    if (field === '$or' && Array.isArray(condition)) {
      const subPredicates = condition.map(subQuery => compileMatch(subQuery as QueryExpression, ctx));
      compiledConditions.push((doc: Document) => subPredicates.some(pred => pred(doc)));
      continue;
    }
    ...
    if (typeof condition !== 'object' || condition === null) {
      // Simple equality
      compiledConditions.push((doc: Document) => fieldGetter(doc) === condition);
    } else {
      // Complex operators
      for (const [operator, expectedValue] of Object.entries(condition)) {
        switch (operator) {
          case '$eq':
            compiledConditions.push((doc: Document) => fieldGetter(doc) === expectedValue);
            break;
          ...
          case '$in':
            if (Array.isArray(expectedValue)) {
              ...
              compiledConditions.push((doc: Document) => valueSet.has(fieldGetter(doc) as any));
            }
            break;
          ...
          case '$regex':
            if (typeof expectedValue === 'string') {
              const regex = compileRegex(expectedValue, condition.$options as string, ctx);
              compiledConditions.push((doc: Document) => {
                const value = fieldGetter(doc);
 ...
 */
export function compileProject(projectSpec: Record<string, any>, ctx: CompilationContext): CompiledProjector {
 ...
 */
export function compileExpression(expression: Expression, ctx: CompilationContext): CompiledExpression {
  ...
  if ('$multiply' in expression && Array.isArray(expression.$multiply) && expression.$multiply.length === 2) {
  ...
  if ('$add' in expression && Array.isArray(expression.$add)) {
    const compiledTerms = expression.$add.map(term => compileExpression(term as Expression, ctx));
 ...
 */
export function canFuseMatchProject(matchQuery: QueryExpression, projectSpec: Record<string, any>): boolean {
  // Fusion is safe if:

...
function estimateCardinality(
  ...
  keyGenerator: (doc: Document) => string
): number {
...
export function fastGroup<T extends Document = Document>(
  ...
  const _estimatedCardinality = estimateCardinality(collection, keyGenerator);
  ...
  for (const doc of collection) {
    ...
    if (!groupKeys.has(groupKey)) {
      ...
      groupIdValues.set(
        groupKey,
        specifications._id ? $expression(doc, specifications._id) : null
      );
    }

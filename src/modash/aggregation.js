            ...
            for (const operator in condition) {
                ...
                switch (operator) {
                    ...
                    case '$nin': 
                        ...
                        break;
                    case '$exists': 
                        const exists = doc.hasOwnProperty(field) && fieldValue !== undefined;
                    ...
                    case '$all':
                        ...
                        if (!expectedValue.every(val => fieldValue.includes(val))) return false;
                        break;
                    case '$elemMatch':
                        if (!Array.isArray(fieldValue)) return false;
                        if (!fieldValue.some(elem => matchDocument({ elem }, { elem: expectedValue }))) return false;
                        break;
                    case '$size':
                        if (!Array.isArray(fieldValue)) return false;
...
function $lookup(collection, { from, localField, foreignField, as }) {
    if (!from || !Array.isArray(from)) {
        throw new Error('$lookup: "from" must be an array (the foreign collection)');
    }
    ...
    return collection.map(doc => {
        ...
        const matches = from.filter(foreignDoc => 
            lodashGet(foreignDoc, foreignField) === localValue
        );
...
export { aggregate, $project, $group, $match, $limit, $skip, $sort, $unwind, $lookup, $addFields, $set };

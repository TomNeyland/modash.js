...
export class NumericKernels {
  ...
  private getOptimalBatchSize(
    size: number,
    operationType: 'alu' | 'memory'
  ): number {
    const threshold =
      operationType === 'alu'
        ? this.ALU_OPTIMAL_BATCH
        : this.MEMORY_OPTIMAL_BATCH;
  ...
  private updateAvgBatchSize(batchSize: number) {
    const totalOps =
      this.stats.batchesProcessed * this.stats.avgBatchSize + batchSize;

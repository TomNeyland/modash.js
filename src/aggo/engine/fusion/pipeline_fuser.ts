...
class StageAnalyzer {
  ...
  private static isSimpleProject(operand: any): boolean {
    ...
    return Object.values(operand).every(
      value =>
        typeof value === 'number' ||
        typeof value === 'string' ||
  ...
  private static hasVectorizableExpressions(operand: any): boolean {
    return Object.values(operand).every(value =>
      this.isVectorizableExpression(value)
    );
  }

  private static hasVectorizableAccumulators(operand: any): boolean {
    const accumulators = Object.keys(operand).filter(key => key !== '_id');
    return accumulators.every(acc =>
      this.isVectorizableAccumulator(operand[acc])
    );
  }
...
export class PipelineFuser {
  ...
  fusePipeline(pipeline: any[]): FusionGroup[] {
    ...
    // Analyze all stages
    const analyzedStages = pipeline.map(stage =>
      StageAnalyzer.analyzePipelineStage(stage)
    );
    ...
    const processedGroups = fusionGroups.map(group =>
      this.processFusionGroup(group)
    );

    // Filter groups that meet criteria
    const validGroups = processedGroups.filter(group =>
      this.isValidFusionGroup(group)
    );
  ...
  generateOptimizedPipeline(originalPipeline: any[]): any[] {
    ...
    for (const group of fusionGroups) {
      if (group.stages.length > 1) {
        ...
        optimizedPipeline.push({
          [group.stages[0].operator]: group.stages[0].operand,
        });
        stageIndex++;
      }
    }
  ...
  private canAddToGroup(
    ...
    if (this.config.vectorizationRequired) {
      const allVectorizable =
        currentGroup.every(s => s.vectorizable) && stage.vectorizable;
      if (!allVectorizable) {
        return false;
      }
    }
    ...
    const totalComplexity =
      currentGroup.reduce((sum, s) => sum + s.complexity, 0) + stage.complexity;
  ...
  private generateFusedOperator(stages: PipelineStage[]): string {
    ...
    for (const stage of stages) {
      switch (stage.operator) {
        ...
        default:
          operations.push(
            `${stage.operator}(${JSON.stringify(stage.operand)})`
          );
      }
    }
  ...
  private estimateSpeedup(stages: PipelineStage[]): number {
    ...
    const projectionStages = stages.filter(
      s => s.operator === '$project' || s.operator === '$addFields'
    );
  ...
  private isCompatibleWithGroupOutput(projection: any, grouping: any): boolean {
    ...
    const groupOutputFields = [
      '_id',
      ...Object.keys(grouping).filter(k => k !== '_id'),
    ];

    return projectionFields.every(
      field =>
        groupOutputFields.includes(field) ||
        projection[field] === 0 ||
  ...
  private updateStats(groups: FusionGroup[]) {
    ...
    this.stats.avgGroupSize =
      groups.length > 0 ? totalStages / groups.length : 0;

    this.stats.totalSpeedup += groups.reduce(
      (sum, g) => sum + g.estimatedSpeedup,
      0
    );
    this.stats.codeGenerated += groups.reduce((sum, g) => sum + g.codeSize, 0);
    this.stats.semanticsPreserved += groups.filter(
      g => g.maintainsSemantics
    ).length;
  }
...
export class PipelineAnalysisUtils {
  ...
  static analyzeFusionOpportunities(pipeline: any[]): {
    ...
    const fusabilityRatio =
      stages.length > 0 ? fusableStages / stages.length : 0;
  ...
  static recommendFusionConfig(pipelines: any[][]): Partial<FusionConfig> {
    ...
    const avgStages =
      analyses.reduce((sum, a) => sum + a.totalStages, 0) / analyses.length;
    const avgComplexity =
      analyses.reduce((sum, a) => sum + a.complexityScore, 0) / analyses.length;
    const avgFusability =
      analyses.reduce((sum, a) => sum + a.fusabilityRatio, 0) / analyses.length;

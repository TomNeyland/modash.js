...
export class TrigramPrefilter {
  ...
  private sessions = new Map<
    string,
    { lastAccess: number; queryCount: number }
  >();
  ...
  addString(str: string): number {
    ...
    const limitedTrigrams = Array.from(trigrams).slice(
      0,
      this.config.maxTrigramsPerString
    );
  ...
  searchSubstring(query: SubstringQuery): number[] {
    ...
    this.stats.candidatesFiltered =
      this.stats.candidatesGenerated - filteredIds.length;
  ...
  getSessionInfo(): Map<
    string,
    { queryCount: number; lastAccess: number; queries: SubstringQuery[] }
  > {
  ...
  private updateStats() {
    ...
    this.stats.averageTrigramsPerString =
      this.stats.totalStrings > 0
  ...
  private updateFilterEfficiency() {
    if (this.stats.candidatesGenerated > 0) {
      this.stats.filterEfficiency =
        this.stats.candidatesFiltered / this.stats.candidatesGenerated;
    }
  }
...
export class TrigramPrefilterManager {
  ...
  private queryPatterns = new Map<
    string,
    { count: number; patterns: Set<string> }
  >();
  ...
  getCombinedStats() {
    ...
    combined.avgFilterEfficiency =
      prefiltersWithQueries > 0 ? totalEfficiency / prefiltersWithQueries : 0;

...
export class BloomFilter {
  ...
  private calculateOptimalParameters(
    ...
    const hashFunctions = Math.ceil(
      (bitCount / expectedElements) * Math.log(2)
    );
...
export class JoinBloomFilterManager {
  ...
  getCombinedStats() {
    ...
    combined.avgFalsePositiveRate =
      this.filters.size > 0 ? totalFPR / this.filters.size : 0;
...
export class BloomFilterUtils {
  ...
  static recommendConfig(
    buildSideSize: number,
    probeRatio: number = 0.1
  ): BloomFilterConfig {

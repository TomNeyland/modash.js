...
export class ZoneMap {
  ...
  addValue(value: any) {
    ...
    if (this.config.supportedTypes.includes(valueType) && valueType === this.stats.dataType) {
  ...
  canSkipForRange(operator: string, compareValue: any): FilterResult {
    if (this.stats.min === undefined || this.stats.max === undefined) {
      return { canSkip: false, reason: 'No statistics available', confidence: 0 };
    }
    ...
    switch (operator) {
      case '$gt':
        if (this.stats.max <= compareValue) {
          return { canSkip: true, reason: 'Max value <= compare value', confidence: 1.0 };
        }
        break;
      ...
      case '$lt':
        if (this.stats.min >= compareValue) {
          return { canSkip: true, reason: 'Min value >= compare value', confidence: 1.0 };
        }
        break;
      ...
      case '$eq':
        if (compareValue < this.stats.min || compareValue > this.stats.max) {
          return { canSkip: true, reason: 'Value outside min/max range', confidence: 1.0 };
        }
        break;
        
      case '$ne':
        // Can only skip if chunk contains only one unique value equal to compareValue
        if (this.stats.min === this.stats.max && this.stats.min === compareValue && !this.stats.hasNulls) {
          return { canSkip: true, reason: 'Chunk contains only the excluded value', confidence: 1.0 };
        }
        break;
      ...
      case '$in':
        if (Array.isArray(compareValue)) {
          ...
          if (this.stats.max < minInList || this.stats.min > maxInList) {
            return { canSkip: true, reason: 'Chunk range outside $in list range', confidence: 1.0 };
          }
        }
        break;
      ...
      case '$nin':
        if (Array.isArray(compareValue)) {
          ...
          if (this.stats.min >= minInList && this.stats.max <= maxInList && 
              this.stats.min === this.stats.max && compareValue.includes(this.stats.min)) {
            return { canSkip: true, reason: 'Chunk value is in exclusion list', confidence: 1.0 };
          }
        }
        break;
    }
    
    return { canSkip: false, reason: 'Cannot determine skip condition', confidence: 0 };
  }
  ...
  canSkipForExists(exists: boolean): FilterResult {
    if (exists) {
      ...
      if (this.stats.totalCount === this.stats.nullCount) {
        return { canSkip: true, reason: 'Chunk contains only nulls', confidence: 1.0 };
      }
    } else {
      // Looking for null values
      if (!this.stats.hasNulls) {
        return { canSkip: true, reason: 'Chunk contains no nulls', confidence: 1.0 };
      }
    }
  ...
  merge(other: ZoneMap): ZoneMap {
    ...
    merged.stats.dataType = this.stats.dataType !== 'unknown' ? this.stats.dataType : otherStats.dataType;
    
    // Merge min/max values
    if (this.stats.min !== undefined && otherStats.min !== undefined) {
      merged.stats.min = this.compareValues(this.stats.min, otherStats.min) <= 0 ? this.stats.min : otherStats.min;
      merged.stats.max = this.compareValues(this.stats.max, otherStats.max) >= 0 ? this.stats.max : otherStats.max;
  ...
  private updateMinMax(value: any) {
    if (this.stats.min === undefined || this.compareValues(value, this.stats.min) < 0) {
      this.stats.min = value;
    }
    
    if (this.stats.max === undefined || this.compareValues(value, this.stats.max) > 0) {
...
export class ZoneMapManager {
   ...
   */
  getSkippableChunks(columnName: string, operator: string, value: any): number[] {
  ...
  getAllStats(): Map<string, ZoneMapStats[]> {
    ...
    for (const [columnName, zoneMaps] of this.zoneMaps) {
      allStats.set(columnName, zoneMaps.map(zm => zm.getStats()));
    }
...
export class ZoneMapUtils {
   ...
   */
  static estimateSelectivity(zoneMap: ZoneMapStats, operator: string, value: any): number {
  ...
  static analyzeColumn(values: any[]): {
    ...
    const dataType = nonNullValues.length > 0 ? typeof nonNullValues[0] : 'unknown';
